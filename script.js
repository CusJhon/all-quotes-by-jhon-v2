...
const formatsV2 = [
    ...
    { id: 'cekpanyakit', name: 'Cek Potensi Penyakit', icon: 'fas fa-notes-medical', endpoint: '/api/primbon/cek_potensi_penyakit', baseUrl: 'https://api.siputzx.my.id', params: [{ name: 'tgl', label: 'Tanggal Lahir', type: 'text' }, { name: 'bln', label: 'Bulan Lahir', type: 'text' }, { name: 'thn', label: 'Tahun Lahir', type: 'text' }] },
    { id: 'random_bluearchive', name: 'Random Blue Archive', icon: 'fas fa-dice-d6', endpoint: '/api/r/blue-archive', baseUrl: 'https://api.siputzx.my.id', params: [] },
    { id: 'random_cecan_thailand', name: 'Random Cecan Thailand', icon: 'fas fa-dice-d6', endpoint: '/api/r/cecan/thailand', baseUrl: 'https://api.siputzx.my.id', params: [] },
    { id: 'random_cecan_china', name: 'Random Cecan China', icon: 'fas fa-dice-d6', endpoint: '/api/r/cecan/china', baseUrl: 'https://api.siputzx.my.id', params: [] },
    { id: 'random_cecan_vietnam', name: 'Random Cecan Vietnam', icon: 'fas fa-dice-d6', endpoint: '/api/r/cecan/vietnam', baseUrl: 'https://api.siputzx.my.id', params: [] },
    { id: 'random_cecan_indonesia', name: 'Random Cecan Indonesia', icon: 'fas fa-dice-d6', endpoint: '/api/r/cecan/indonesia', baseUrl: 'https://api.siputzx.my.id', params: [] },
    { id: 'random_cecan_japan', name: 'Random Cecan Japan', icon: 'fas fa-dice-d6', endpoint: '/api/r/cecan/japan', baseUrl: 'https://api.siputzx.my.id', params: [] },
    { id: 'random_cecan_korea', name: 'Random Cecan Korea', icon: 'fas fa-dice-d6', endpoint: '/api/r/cecan/korea', baseUrl: 'https://api.siputzx.my.id', params: [] },
    { id: 'random_waifu', name: 'Random Waifu', icon: 'fas fa-dice-d6', endpoint: '/api/r/waifu', baseUrl: 'https://api.siputzx.my.id', params: [] },
    { id: 'random_neko', name: 'Random Anime Neko', icon: 'fas fa-dice-d6', endpoint: '/api/r/neko', baseUrl: 'https://api.siputzx.my.id', params: [] }
];
...
function renderParamForm(prefix, activeFormat) {
    ...
    if (!container) return;
    if (!activeFormat.params?.length) { container.innerHTML = '<div class="input-group"><p style="color:#9ca3af;">Tidak ada parameter</p></div>'; return; }
...
async function generateMockup(version) {
    ...
    const { interval, fill, percent } = showGenerateLoading(version);
    ...
    try {
        ...
        if (downloadBtn) { const newBtn = downloadBtn.cloneNode(true); downloadBtn.parentNode.replaceChild(newBtn, downloadBtn); newBtn.onclick = async (e) => { e.preventDefault(); if (typeof window.downloadImage === 'function') await window.downloadImage(imageUrl, `${activeFormat.id}_mockup_${Date.now()}.png`); else { const a = document.createElement('a'); a.href = imageUrl; a.download = `${activeFormat.id}_mockup_${Date.now()}.png`; a.click(); } }; }
        hideGenerateLoading(interval, fill, percent, true, version);
        if (window.showToast) window.showToast('Mockup berhasil dibuat!', 'success');
    } catch (err) { resultDiv.innerHTML = `<div class="result-placeholder"><i class="fas fa-times-circle"></i><p>Gagal: ${err.message}</p></div>`; hideGenerateLoading(interval, fill, percent, false, version); if (window.showToast) window.showToast(err.message, 'error'); }
}

// ========== SSWEB FUNCTIONS ==========
async function generateSSWeb() {
    const url = document.getElementById('sswebUrl')?.value, device = document.getElementById('sswebDevice')?.value, fullPage = document.getElementById('sswebFullPage')?.value, scale = document.getElementById('sswebScale')?.value;
    ...
    const resultDiv = document.getElementById('sswebResultArea'), generateBtn = document.getElementById('sswebGenerateBtn'), loadingDiv = document.getElementById('sswebLoading'), loadingFill = document.getElementById('sswebProgressFill'), loadingPercent = document.getElementById('sswebProgressPercent'), loadingMessage = document.getElementById('sswebLoadingMessage');
    generateBtn.classList.add('hidden'); loadingDiv.classList.remove('hidden');
    loadingFill.style.width = '0%'; loadingPercent.textContent = '0'; loadingMessage.innerHTML = '<i class="fas fa-globe"></i> Menghubungkan ke server...';
    let progress = 0, msgIndex = 0;
    const loadingMessages = [{ icon: 'fa-globe', text: 'Menghubungkan ke server...' }, { icon: 'fa-camera', text: 'Mengambil screenshot...' }, { icon: 'fa-image', text: 'Memproses gambar...' }, { icon: 'fa-check-circle', text: 'Menyelesaikan...' }];
    const progressInterval = setInterval(() => {
        if (progress < 90) { progress += Math.random() * 15; if (progress > 90) progress = 90; loadingFill.style.width = progress + '%'; loadingPercent.textContent = Math.floor(progress); }
    ...
    let apiUrl = `https://api.zenzxz.my.id/tools/ssweb?url=${encodeURIComponent(url)}&device=${device}&full_page=${fullPage}&scale=${scale}`;
    try {
        const response = await fetch(apiUrl); if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const data = await response.json(); if (!data.status || !data.result?.url) throw new Error(data.message || 'Gagal mengambil screenshot');
        const imageUrl = data.result.url;
        clearInterval(progressInterval); loadingFill.style.width = '100%'; loadingPercent.textContent = '100'; loadingMessage.innerHTML = '<i class="fas fa-check-circle"></i> Selesai!';
        setTimeout(() => {
            ...
            if (downloadBtn) { const newBtn = downloadBtn.cloneNode(true); downloadBtn.parentNode.replaceChild(newBtn, downloadBtn); newBtn.onclick = async (e) => { e.preventDefault(); if (typeof window.downloadImage === 'function') await window.downloadImage(imageUrl, `screenshot_${Date.now()}.png`); else { const a = document.createElement('a'); a.href = imageUrl; a.download = `screenshot_${Date.now()}.png`; a.click(); } }; }
            loadingDiv.classList.add('hidden'); generateBtn.classList.remove('hidden');
        }, 500);
        if (window.showToast) window.showToast('Screenshot berhasil diambil!', 'success');
    } catch (err) { clearInterval(progressInterval); resultDiv.innerHTML = `<div class="result-placeholder"><i class="fas fa-times-circle"></i><p>❌ Gagal: ${err.message}</p></div>`; loadingDiv.classList.add('hidden'); generateBtn.classList.remove('hidden'); if (window.showToast) window.showToast(err.message, 'error'); }
}
